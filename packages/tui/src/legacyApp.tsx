...
type NodeTemplateKey = typeof NODE_TEMPLATE_KEYS[number];

const NODE_TEMPLATE_HINT = NODE_TEMPLATE_KEYS.join('|');
...
function normalizeNodeTemplateKey(input: string): NodeTemplateKey | undefined {
...
function buildAddNodeTemplate(nodeType: NodeTemplateKey, nextId: string): string {
    if (nodeType === 'action') {
        return [
            `type: action`,
...
export function LegacyApp(props: AppProps): JSX.Element {
    const { exit } = useApp();
    ...
    useInput((input, key) => {
        ...
        if (tab === 'editor') {
            ...
            if (input === 'a') {
                openPrompt(
                    `Add node type (${NODE_TEMPLATE_HINT})`,
                    'action',
                    (value) => {
                        const templateKey = normalizeNodeTemplateKey(String(value || ''));
                        if (!templateKey) {
                            setStatusLine(`Unknown node type: "${String(value || '')}"`);
                            return;
                        }
                        const nextId = `${templateKey.replace(/[^a-z0-9]+/g, '_')}_${Date.now().toString(36).slice(-6)}`;
                        const yamlPayload = buildAddNodeTemplate(templateKey, nextId);
                        setStatusLine(`Opening ${templateKey} template in YAML editor...`);
                        ...
                        try {
                            ...
                            setStatusLine(`Node added (${templateKey}).`);
                            refreshEditorNodes();
                            refreshPipelines();
                        } catch (error: any) {
            ...
            {tab === 'editor' && (
                <Box marginTop={1}>
                    <Box flexDirection="column" width="45%" borderStyle="round" borderColor="green" paddingX={1} marginRight={1}>
                        ...
                        <Text color="gray">↑/↓ select | a add (catalog + yaml) | y edit node yaml | v full pipeline yaml</Text>
                    ...
                    <Box flexDirection="column" width="55%" borderStyle="round" borderColor="blue" paddingX={1}>
                        ...
                        <Text color="blueBright">{sectionTitle('Add Node Catalog')}</Text>
                        <Text color="gray">action | script | http | prompt | form | switch | repo</Text>
                        <Text color="gray">sub_pipeline | loop | agent | team | memory_save</Text>
                        <Text color="gray">memory_recall | memory_clear</Text>
                        <Text color="gray">editor: {resolveEditorCommand()}</Text>
                    </Box>
                </Box>
            )}
